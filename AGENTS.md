# Agent Guidelines for Foundatio.AWS
...
```bash
# Build
dotnet build Foundatio.AWS.slnx

# Test
dotnet test Foundatio.AWS.slnx

# Format code
dotnet format Foundatio.AWS.slnx
```
...
```text
src
└── Foundatio.AWS                    # AWS provider implementations
    ├── Queues                       # SQS queue implementations
    ├── Storage                      # S3 storage implementations
    └── Extensions                   # Extension methods for DI
tests
└── Foundatio.AWS.Tests              # Unit and integration tests
```
...
1. **Builds successfully**: `dotnet build Foundatio.AWS.slnx` exits with code 0
2. **All tests pass**: `dotnet test Foundatio.AWS.slnx` shows no failures
...
- Mirror the main code structure (e.g., `Queues/` tests for `src/Foundatio.AWS/Queues/`)
...
# Specific test file
dotnet test --filter "FullyQualifiedName~SQSQueueTests"
